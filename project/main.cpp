 ...
 */
void doSearch(Graph &startGraph, SearchStructure &searchStructure) {
    int graphsCount = 1;
    ...
    searchStructure.put(new Graph(startGraph)); // make a copy of the graph so that it can be safely deleted during dfs

    while (!searchStructure.empty()) {
        Graph *graph = searchStructure.take();
